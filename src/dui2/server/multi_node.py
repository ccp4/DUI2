...
class CmdNode(object):
    ...
    def generate_predict_n_report(self, req_obj = None):
        ...
        # if dials.(scale/symmetry/merge/cosym) already generated an html file
        # Dui2 should use it instead of running dials.report

        tmp_scale_html_path = self._run_dir + os.sep + "dials.scale.html"
        tmp_symmetry_html_path = self._run_dir + os.sep + "dials.symmetry.html"
        ...
        if os.path.exists(tmp_scale_html_path):
            ...
            logging.info("Report already generated by the 'scale' cmd tool")

        elif os.path.exists(tmp_symmetry_html_path):
            shutil.copy(tmp_symmetry_html_path, tmp_html_path)
            logging.info("Report already generated by the 'symmetry' cmd tool")

        elif os.path.exists(tmp_cosym_html_path):
            shutil.copy(tmp_cosym_html_path, tmp_html_path)
            logging.info("Report already generated by the 'cosym' cmd tool")

        elif os.path.exists(tmp_merge_html_path):
            shutil.copy(tmp_merge_html_path, tmp_html_path)
            logging.info("Report already generated by the 'merge' cmd tool")
